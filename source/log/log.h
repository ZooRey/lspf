...
#ifndef LSPF_LOG_LOG_H
#define LSPF_LOG_LOG_H
...
typedef boost::function<void(int pri, const char* file, uint32_t line,
    const char* function, const char *id, const char* msg)> TLogFunc;

typedef boost::function<void(void)> TLogFlushFunc;

class Log {
public:
    /// @brief 设置log id--日志跟踪标识
    /// @para id 日志ID
    static void SetLogId(const std::string &id);

    /// @brief 获取log id--日志跟踪标识
    /// @return id--日志跟踪标识
    ...
    static void RegisterTlogFunc(const TLogFunc& tlog_func, const TLogFlushFunc &tlog_flush_func = NULL);
...
#define PLOG_INFO(fmt, ...)  lspf::log::Log::Write(lspf::log::LOG_PRIORITY_INFO,  __FILE__, __LINE__, __FUNCTION__, 0, 0, fmt, ##__VA_ARGS__); // NOLINT
#define PLOG_DEBUG(fmt, ...) lspf::log::Log::Write(lspf::log::LOG_PRIORITY_DEBUG, __FILE__, __LINE__, __FUNCTION__, 0, 0, fmt, ##__VA_ARGS__); // NOLINT
#define PLOG_TRACE(fmt, ...) lspf::log::Log::Write(lspf::log::LOG_PRIORITY_TRACE, __FILE__, __LINE__, __FUNCTION__, 0, 0, fmt, ##__VA_ARGS__); // NOLINT
...
// 支持过滤，只适用于使用tlog
#define PLOG_FILTER_FATAL(id, fmt, ...) lspf::log::Log::Write(lspf::log::LOG_PRIORITY_FATAL, __FILE__, __LINE__, __FUNCTION__, id, 0, fmt, ##__VA_ARGS__); // NOLINT
#define PLOG_FILTER_ERROR(id, fmt, ...) lspf::log::Log::Write(lspf::log::LOG_PRIORITY_ERROR, __FILE__, __LINE__, __FUNCTION__, id, 0, fmt, ##__VA_ARGS__); // NOLINT
#define PLOG_FILTER_INFO(id, fmt, ...)  lspf::log::Log::Write(lspf::log::LOG_PRIORITY_INFO,  __FILE__, __LINE__, __FUNCTION__, id, 0, fmt, ##__VA_ARGS__); // NOLINT
#define PLOG_FILTER_DEBUG(id, fmt, ...) lspf::log::Log::Write(lspf::log::LOG_PRIORITY_DEBUG, __FILE__, __LINE__, __FUNCTION__, id, 0, fmt, ##__VA_ARGS__); // NOLINT
#define PLOG_FILTER_TRACE(id, fmt, ...) lspf::log::Log::Write(lspf::log::LOG_PRIORITY_TRACE, __FILE__, __LINE__, __FUNCTION__, id, 0, fmt, ##__VA_ARGS__); // NOLINT

/*
// 支持过滤，只适用于使用tlog
#define PLOG_FILTER_FATAL(id, cls, fmt, ...) Log::Write(LOG_PRIORITY_FATAL, __FILE__, __LINE__, __FUNCTION__, id, cls, fmt, ##__VA_ARGS__); // NOLINT
#define PLOG_FILTER_ERROR(id, cls, fmt, ...) Log::Write(LOG_PRIORITY_ERROR, __FILE__, __LINE__, __FUNCTION__, id, cls, fmt, ##__VA_ARGS__); // NOLINT
#define PLOG_FILTER_INFO(id, cls, fmt, ...)  Log::Write(LOG_PRIORITY_INFO,  __FILE__, __LINE__, __FUNCTION__, id, cls, fmt, ##__VA_ARGS__); // NOLINT
#define PLOG_FILTER_DEBUG(id, cls, fmt, ...) Log::Write(LOG_PRIORITY_DEBUG, __FILE__, __LINE__, __FUNCTION__, id, cls, fmt, ##__VA_ARGS__); // NOLINT
#define PLOG_FILTER_TRACE(id, cls, fmt, ...) Log::Write(LOG_PRIORITY_TRACE, __FILE__, __LINE__, __FUNCTION__, id, cls, fmt, ##__VA_ARGS__); // NOLINT
*/
} //namespace lspf
} //namespace log
