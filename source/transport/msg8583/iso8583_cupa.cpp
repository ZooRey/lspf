...
//cupa协议规定 11域和41域（受卡方终端标识码）、42域（受卡方标识码）、60.2域（批次号)、消息类型一起匹配原始请求消息
//但是签到的消息的批次号可能不一样，所以不匹配60.2域，同时消息类型只匹配前面两个字节
const unsigned int key_field_array[] = {11, 41, 42};

//
const unsigned int ISO8583_CUPA_style_array[BITMAP_LEN*8] =
{
/*
...
ISO8583_CUPA::ISO8583_CUPA()
{
    m_vec_item_style.assign(ISO8583_CUPA_style_array, ISO8583_CUPA_style_array + BITMAP_LEN*8);
}
...
string ISO8583_CUPA::GetInfo()
{
    return "ISO8583_CUPAV20171030";
}


/////////////////////////////////////////////////////
void ISO8583_CUPA::getStyle(ItemStyle &item_style, unsigned int index)
{
...
int ISO8583_CUPA::decoding(unsigned int index, unsigned int &offset, const ItemStyle &item_style, string &dest)
{
    int i_item_len = 0;
...
int ISO8583_CUPA::encoding(unsigned int index, const string &src, const ItemStyle &item_style, string &dest)
{
    int i_item_len = 0;
...
void ISO8583_CUPA::dealMsgHeader(const string &header)
{
...
int ISO8583_CUPA::Pack(string &msg, bool mac_flag)
{
    int ret = 0;
    string header;
    string header_bcd;
...
int ISO8583_CUPA::Unpack(const string &msg)
{
    int ret = 0;
    int len = 0;
...
string ISO8583_CUPA::GetMsgKey()
{
    int array_len;
    string key;
    key = m_msg_type.substr(0,2);
