...
void ZooKeeperClient::post(const String_vector &strings)
{
	...
	for(i=0; i<strings.count; i++)
	{
		...
		path = "/" + m_service_name + "/" + string(strings.data[i]);
		ret = zoo_get(m_zk_handle, path.c_str(), 0, buffer, &buffer_len, NULL);
		cout << "zoo_get ret=" << ret << endl;
		if (ret == ZOK)
		{
	...
	cout << "m_server_infos->size=" << m_server_infos->size() << endl;	 
}
...
int ZooKeeperClient::GetChildren()
{
	...
	cout << "zoo_wget_children2 ret=" << ret << endl;
	if (ret != ZOK)
	{
		return ret;
	}
